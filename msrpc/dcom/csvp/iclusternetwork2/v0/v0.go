...
type ClusterNetwork2Client interface {
	...
	// The GetAddRoutesStatus method<29> retrieves packet loss information and status for
	...
	// The CancelAddRoutesRequest method<30> stops packet loss and status monitoring for
...
type SendRTMessageRequest struct {
	...
	// in the form specified by [RFC1924].<23>
	SourceIPAddress *oaut.String `idl:"name:SourceIPAddress" json:"source_ip_address"`
	...
	MessageSize uint32 `idl:"name:MessageSize" json:"message_size"`
	// Timeout: An implementation-specific value<24> indicating the maximum amount of time
...
type GetIPConfigSerializedRequest struct {
	...
	// field of the ADAPTER2 <25> structure), loopback adapters, and tunnel adapters.
	//
	//	+----------------+----------------------------------+
	//	|                |                                  |
	//	|     VALUE      |             MEANING              |
...
type GetIPConfigSerializedResponse struct {
	// That: ORPCTHAT structure that is used to return ORPC extension data to the client.
	That *dcom.ORPCThat `idl:"name:That" json:"that"`
	// Data: A buffer that, on success, SHOULD<26> contain a valid ADAPTERLIST2 structure.
...
type QueryFirewallConfigurationResponse struct {
	...
	// "Failover Cluster Manager"<27> is enabled.
	//
	//	+----------------+------------------------------------------------------------------+
	//	|                |                                                                  |
	//	|     VALUE      |                             MEANING                              |
