...
type GetConfigManagerResponse struct {
	// That: ORPCTHAT structure that is used to return ORPC extension data to the client.
	That *dcom.ORPCThat `idl:"name:That" json:"that"`
	// ppConfigManager: If the server successfully processes the message that is received
	// from the client, ppConfigManager contains a pointer to an IAppHostConfigManager object.
	ConfigManager *iisa.AppHostConfigManager `idl:"name:ppConfigManager" json:"config_manager"`
