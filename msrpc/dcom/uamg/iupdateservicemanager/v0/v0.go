...
type SetOptionRequest struct {
	...
	//
	// If optionName is "AllowWarningUI", the vt member MUST be set to VT_BOOL, as specified
	// in [MS-OAUT] section 2.2.7, and the boolVal member MUST be set to VARIANT_TRUE if
	// a warning UI is allowed to be displayed when changing the service registered with
	// the automatic update agent, or VARIANT_FALSE if a warning UI is not allowed to be
	// displayed.
	OptionValue *oaut.Variant `idl:"name:optionValue" json:"option_value"`
}
