...
type StatusRequest struct {
	...
	//
	// If the dwBufferSize parameter value indicates that pbBuffer is too small to contain
	// all the status information about the Internet services, the following actions MUST
	// occur:
	//
...
type StatusResponse struct {
	...
	//
	// pbBuffer MAY be set to null. In this case, the size will be calculated by the system
	// for the buffer (regardless of whether a size was passed in for the buffer size) and
	// E_ERROR_INSUFFICIENT_BUFFER will be returned. If pdwMDRequiredBufferSize is not null,
	// it will be used to return the calculated size.
	Buffer []byte `idl:"name:pbBuffer;size_is:(dwBufferSize)" json:"buffer"`
