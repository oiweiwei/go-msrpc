...
type RouterInterfaceTransportSetGlobalInfoRequest struct {
	...
	//
	// The pGlobalInfo member MUST point to a valid RTR_INFO_BLOCK_HEADER (section 2.2.1.2.3)
	// and RTR_TOC_ENTRY (section 2.2.1.2.4). Otherwise, an error code is returned. The
	// acceptable RTR_TOC_ENTRY values depend on the transport and the RRAS server implementation
	// support; hence the RRAS server implementation SHOULD check that InfoType of RTR_TOC_ENTRY
	// is supported.
	Info *rrasm.InterfaceContainer `idl:"name:pInfoStruct" json:"info"`
}
...
type RouterInterfaceCreateRequest struct {
	...
	// dwfOptions member, then the dwEncryptionType member of the MPRI_INTERFACE_2 or MPRI_INTERFACE_3
	// structures SHOULD be initialized to MPR_ET_Require and the value of the dwfOptions
	// member SHOULD be modified to enable the following flags by default:
	//
...
type RouterInterfaceGetInfoRequest struct {
	...
	// values.
	//
	// The dwfOptions member of MPRI_INTERFACE_2 and MPRI_INTERFACE_3 SHOULD be updated
	// as follows:
	//
...
type RouterInterfaceSetInfoRequest struct {
	...
	//
	// When the dwLevel value is either 2 or 3 and the dwfOptions member does not contain
	// any of the flags MPRIO_RequirePAP, MPRIO_RequireCHAP, MPRIO_RequireMsCHAP2, or MPRIO_RequireEAP,
	// then the dwfOptions value SHOULD be modified to enable MPRIO_RequireMsCHAP2, MPRIO_RequireCHAP,
	// and MPRIO_RequirePAP.
	Info *rrasm.InformationContainer `idl:"name:pInfoStruct" json:"info"`
	// hInterface: The unique identifier of an interface. This can be obtained from RRouterInterfaceCreate
...
type RouterInterfaceConnectRequest struct {
	...
	//
	// If this parameter is set to 0, the function will return immediately. A return value
	// of PENDING (0x00000258) indicates that the connection attempt was successfully initiated.
	Blocking uint32 `idl:"name:fBlocking" json:"blocking"`
...
type MIBEntryCreateRequest struct {
	...
	// (0x00002710), the pMibInEntry field of pInfoStruct MUST be a pointer to MIB_OPAQUE_INFO
	// (section 2.2.1.2.52). The dwId and rgbyData fields of MIB_OPAQUE_INFO MUST be set
...
type MIBEntryDeleteRequest struct {
	...
	//
	// If dwMibInEntrySize does not match the size of MIB_IPDESTROW (section 2.2.1.2.20)
	// an error other than those specified in the return value table is returned.
	Info *rrasm.MIBEntryContainer `idl:"name:pInfoStuct" json:"info"`
}
...
type ConnectionRemoveQuarantineRequest struct {
	...
	//
	// Alternatively, this parameter specifies the IP address of the RRAS client for which
	// to remove the quarantine filter. The IP address MUST be specified as a DWORD in network
	// byte order. Obtain the IP address by calling RRasAdminConnectionEnum. If this parameter
	// specifies an IP address, the fIsIpAddress parameter MUST specify a TRUE value.
	Connection uint32 `idl:"name:hRasConnection" json:"connection"`
