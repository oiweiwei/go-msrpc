...
// computers. This protocol supersedes the Disk Management Remote Protocol, specified
// in [MS-DMRP].
//
// # Overview
//
// The Virtual Disk Service (VDS) Remote Protocol is a set of Distributed Component
// Object Model (DCOM) interfaces for managing the configuration of disk storage on
// a computer. VDS provides a mechanism for remote configuration of disks, partitions,
// volumes, and iSCSI initiators on a server. Through the Virtual Disk Service Remote
// Protocol, a client can change the configuration of disks into partitions, partitions
// into volumes, and volumes into file systems. The protocol also enables clients to
// obtain notifications of changes to these storage objects.
//
...
var (
	...
	// VDSBusTypeMmc:  Indicates a multimedia card (MMC) bus type. For more information
	// about multimedia cards, which are a flash memory card standard, see [JEDEC-MO227-A].
...
var (
	...
	//
	// The disk will be in a read-only state if its read-only attribute is set. However,
	// a disk can be in a read-only state even if its read-only attribute is not set, if
	// the underlying hardware is read-only. For example, if the LUN is in read-only state
	// or if the disk is a virtual hard disk that resides on a volume that is read-only,
	// the underlying hardware is read-only and therefore the disk is in a read-only state.
...
var (
	...
	//
	// VDS_VF_HIDDEN, VDF_VF_READ_ONLY, VDS_VF_SHADOW_COPY, and VDS_VF_NO_DEFAULT_DRIVE_LETTER
	// are set as follows:
	//
...
type VolumeNotification struct {
	// ulEvent: Determines the volume event for which an application will be notified; it
...
var (
	...
	//
	// VDSDiskOfflineReasonResourceExhaustion<43>: The disk is offline because of lack of
	// capacity.
	//
...
var (
	...
	// VDS_SP_ONLINE: All newly discovered disks are brought online and made WRITABLE. If
	// the disk is offline, no volume devices are exposed for the disk. If the disk is online,
	...
	// VDS_SP_OFFLINE_SHARED: All newly discovered disks not residing on a shared bus are
	// brought online and made WRITABLE. If the disk is offline, no volume devices are exposed
	// for the disk. If the disk is online, the volume devices for the disk are exposed.
	// WRITABLE is the normal state for a disk. A disk can also be made READONLY. If the
	// disk is READONLY, disk data and metadata can be read, but writes to the disk will
	// fail.
	SANPolicyOfflineShared SANPolicy = 2
	// VDS_SP_OFFLINE: All newly discovered disks remain offline and READONLY. If the disk
	// is offline, no volume devices are exposed for the disk. If the disk is online, the
	// volume devices for the disk are exposed. WRITABLE is the normal state for a disk.
	// A disk can also be made READONLY. If the disk is READONLY, disk data and metadata
	// can be read, but writes to the disk will fail.
	SANPolicyOffline SANPolicy = 3
	// VDS_SP_OFFLINE_INTERNAL:<31> All newly discovered internal disks remain offline and
	// READONLY. If the disk is offline, no volume devices are exposed for the disk. If
	// the disk is online, the volume devices for the disk are exposed. WRITABLE is the
...
type DiskProperty2 struct {
	...
	Flags uint32 `idl:"name:ulFlags" json:"flags"`
	// BusType: The value from the VDS_STORAGE_BUS_TYPE enumeration that indicates the type
	// of bus where the disk resides.
	BusType StorageBusType `idl:"name:BusType" json:"bus_type"`
	// PartitionStyle: The value from the VDS_PARTITION_STYLE enumeration that indicates
	// the partitioning format of the disk.
	PartitionStyle PartitionStyle               `idl:"name:PartitionStyle" json:"partition_style"`
	...
	Name string `idl:"name:pwszName;string" json:"name"`
	// pwszFriendlyName: The null-terminated Unicode friendly (human-readable) name of the
	// disk as assigned by the operating system. This property MAY be NULL. If present,
	// a client can use this property to display a human-readable name of the disk.
	FriendlyName string `idl:"name:pwszFriendlyName;string" json:"friendly_name"`
	// pwszAdaptorName: The null-terminated Unicode name that the operating system assigns
	// to the adapter to which the disk is attached. This property MAY be NULL. If present,
...
var (
	...
	//
	// For example, if the CREATE_VIRTUAL_DISK_FLAG_NONE flag is used to create a virtual
	// disk that is 2 gigabytes in size, the initial size of the virtual disk file backing
	// store can only be 2 megabytes. As data is written to this disk, the virtual disk
	// file backing store increases in size, with a maximum size of 2 gigabytes.
...
type CreateVDiskParameters struct {
	...
	//
	// A differencing disk represents the current state of the virtual disk as a set of
	// modified blocks in comparison to a parent disk. A differencing disk is not independent;
	// it depends on the parent disk to be fully functional. A differencing disk MUST be
	// created using the CREATE_VIRTUAL_DISK_FLAG_NONE flag. The parent disk can be created
	// using either the CREATE_VIRTUAL_DISK_FLAG_NONE or CREATE_VIRTUAL_DISK_FLAG_FULL_PHYSICAL_ALLOCATION
