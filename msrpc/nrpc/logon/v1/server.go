...
type LogonServer interface {
	...
	//
	// Message processing<224> is identical to NetrLogonSamLogonEx, as specified in section
	...
	// The NetrLogonSamLogoff method SHOULD<225> update the user lastLogoff attribute for
	// the SAM accounts.
	//
	...
	// The NetrServerReqChallenge method SHOULD<180> receive a client challenge and return
	...
	// The NetrServerPasswordSet method SHOULD<192> set a new one-way function (OWF) of
	...
	// The NetrDatabaseDeltas method SHOULD<227> return a set of changes (or deltas) performed
	...
	// parameter. The server SHOULD <229> ( 0c858a52-732a-43ec-85dd-e44b357c1898#Appendix_A_229
	...
	// The NetrDatabaseSync method<233> is a predecessor to the NetrDatabaseSync2 method
	...
	// The NetrGetDCName method MAY<162> be used to retrieve the NetBIOS name of the PDC
	...
	//	+--------------------------------+---------------------------------+
	//
	// Verify that the client has sufficient privileges. The server determines if the client
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<163>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	//
	// If the DomainName parameter is not NULL and is not a valid NetBIOS name format, the
	// server MUST return NERR_DCNotFound ([MS-ERREF] section 2.2).
	...
	// 3.5.4.9.1). All parameters of this method SHOULD<270> have the same meanings as the
	...
	// apply. Extra restrictions are applied to the values of the FunctionCode<271> and
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<166>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	...
	// The NetrDatabaseSync2 method SHOULD<230> return a set of all changes applied to the
	...
	// of the PreferredMaximumLength parameter. The server SHOULD <232> ( 0c858a52-732a-43ec-85dd-e44b357c1898#Appendix_A_232
	...
	// The NetrDatabaseRedo method SHOULD<234> be used by a backup domain controller (BDC)
	...
	// The NetrLogonControl2Ex method SHOULD<253> execute administrative actions that pertain
	...
	// The NetrEnumerateTrustedDomains method SHOULD<237> return a set of NetBIOS names
	// of trusted domains.
	//
	...
	// The method SHOULD<161> return information about a domain controller in the specified
	...
	// after a secure channel has been established.<204>
	//
	// Return Values: The method returns 0x00000000 on success; otherwise, it returns a
	// nonzero error code.
	GetCapabilities(context.Context, *GetCapabilitiesRequest) (*GetCapabilitiesResponse, error)

	// The NetrLogonSetServiceBits method SHOULD<247> be used to notify Netlogon whether
	...
	// The NetrLogonGetTrustRid method SHOULD<241> be used to obtain the RID of the account
	...
	// SHOULD<243> be called by a client computer against a server and is used to compute
	...
	// section. The client SHOULD<245> use this digest to compare against one that is returned
	...
	// The NetrServerAuthenticate3 method SHOULD<181> mutually authenticate the client and
	...
	// method. The method SHOULD<160> return information about a domain controller in the
	...
	// The DsrGetSiteName method SHOULD<168> return the site name for the specified computer
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<169>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	//
	...
	// ) in the domain. The server SHOULD <170> ( 0c858a52-732a-43ec-85dd-e44b357c1898#Appendix_A_170
	...
	// The NetrLogonGetDomainInfo method SHOULD<198> return information that describes the
	...
	// The NetrServerPasswordSet2 method SHOULD<188> allow the client to set a new clear
	// text password for an account used by the domain controller for setting up the secure
	// channel from the client. A domain member SHOULD<189> use this function to periodically
	...
	// changes to the PDC. It SHOULD<246> be used by the client to deliver an opaque buffer
	...
	// The DsrAddressToSiteNamesW method SHOULD<173> translate a list of socket addresses
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<174>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<151>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	...
	// The DsrAddressToSiteNamesExW method SHOULD<175> translate a list of socket addresses
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<176>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	AddressToSiteNamesExW(context.Context, *AddressToSiteNamesExWRequest) (*AddressToSiteNamesExWResponse, error)

	// The DsrGetDcSiteCoverageW method SHOULD<171> return a list of sites covered by a
	...
	// has sufficient privileges (as specified in section 3.5.4.2). If the server is not
	// a domain controller the Access Request mask is set to NETLOGON_WKSTA_RPC_ACCESS.
	// If the server is a domain controller the Access Request mask is set to NETLOGON_DCLOC_RPC_ACCESS.<172>
	// If the client does not have sufficient privileges the server MUST return ERROR_ACCESS_DENIED.
	...
	// The DsrGetForestTrustInformation method SHOULD<239> retrieve the trust information
	...
	// The NetrGetForestTrustInformation method SHOULD<238> retrieve the trust information
	...
	// The NetrServerGetTrustInfo method SHOULD<240> return an information block from a
	...
	// (writable) DC to update to a client's computer account object in Active Directory
