...
type ClientAttachRequest struct {
	...
	//
	// If lProcessId equals 0xFFFFFFFD (-3), pszDomainUser MUST contain the user name. The
	// client in this case is an MMC client that is not interested in events occurring on
...
type ClientAttachResponse struct {
	...
	//
	// If lProcessId equals 0xFFFFFFFD (-3), the server MUST set the value of phAsyncEventsEvent
	// to 0x32323232 for a 32-bit platform or to 0x64646464 for a 64-bit platform.
	AsyncEventsEvent int32 `idl:"name:phAsyncEventsEvent" json:"async_events_event"`
