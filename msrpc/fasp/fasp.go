...
type IPv4Subnet struct {
	...
	//
	// A dwSubNetMask of 0x00000000 is invalid. A subnet mask of 0xFFFFFFFF means that the
	// subnet mask represents a single address.
...
type IPv6Subnet struct {
	...
	// the IPv6 subnet.
	//
	// The dwNumPrefixBits MUST NOT be greater than 128 and not less than 1. The address
	// SHOULD NOT be an unspecified address (an address composed of all zeros),<1> and it
...
var (
	...
	// and MUST NOT be used. It is defined for simplicity in writing IDL definitions and
	// code. This symbolic constant has a value of 0x0010.
	InterfaceTypeMax InterfaceType = 16
	// FW_INTERFACE_TYPE_MAX_V2_23: This value and values that exceed this value are not
	// valid and MUST NOT be used in client and server schema version newer than 0x217.
	// It is defined for simplicity in writing IDL definitions and code. This symbolic constant
	// has a value of 0x0008.
...
//
// This enumeration represents flags that are used to identify trust tuples.<25> The
...
var (
	...
	// and code. This symbolic constant has a value of 0x0100.<26> <27>
...
var (
	...
	// FW_RULE_STATUS_SEMANTIC_ERROR_FLAGS_LOCAL_ONLY_MAPPED_VER: Semantic error: The schema
	// version is not compliant with local-only mappings.
	...
	// FW_RULE_STATUS_SEMANTIC_ERROR_TUNNEL_EXEMPT_WITH_GATEWAY: Semantic error: DTM tunnel
	// exemption specified with tunnel endpoint (gateways) address.
	RuleStatusSemanticErrorTunnelExemptWithGateway RuleStatus = 1048817
	// FW_RULE_STATUS_SEMANTIC_ERROR_TUNNEL_EXEMPT_VER:  Semantic error: Schema version
	// not compliant with tunnel mode exemptions.
	...
	//
	// FW_RULE_STATUS_SEMANTIC_ERROR_CRYPTO_ENCR_HASH:  Semantic error: Both the encryption
	// and hash are not specified.
	...
	//
	// FW_RULE_STATUS_SEMANTIC_ERROR_SCHEMA_VERSION:  Semantic error: The specified schema
	// version is earlier than the supported versions.
	...
	// FW_RULE_STATUS_SEMANTIC_ERROR_BYPASS_TUNNEL_IF_SECURE_ON_TRANSPORT: Semantic error:
	// Cannot set flag to exempt IPsec transport traffic from a tunnel mode, on a transport
	// rule.
...
var (
	...
	//
	// Otherwise, the operating system is not equal to the one specified. This symbolic
	// constant has a value of 0.
	...
	// >= minor version))
	//
	// Otherwise, the operation system is not greater than or equal to the one specified.
...
var (
	...
	//
	// The policy versions referenced for the previous behavior note citation can be translated
	// into binary versions by considering the two-byte hex values as organized with a "high
	// byte" and a "low byte". To obtain the binary version, convert each byte to decimal
	// and separate them with a period ("."). For example, hex firewall policy version 0x0214
	// is mapped to binary version 2.20. Schema versions are similar to binary versions
	// except that they contain an underscore character ("_")  instead of a period. The
	// latter is the format that is used when appending a schema version reference to a
	// structure name or a method name, as described earlier in this section.
	...
	// enumeration, which, in turn, depends on the schema version. See the descriptions
	// of the previous enumeration members to determine what this value is for each schema
	// version.
...
var (
	...
	// keying modules are implementation-specific.<24>
...
type CSRule struct {
	...
	// is enforced by the local computer based on the local computer's platform information.
	// The rule is enforced only if the local computer's operating system platform is an
	// element of the set described by PlatformValidityList.<15>
	PlatformValidityList *OSPlatformList `idl:"name:PlatformValidityList" json:"platform_validity_list"`
...
var (
	...
	// FW_AUTH_SUITE_FLAGS_MAX_V2_10: This value and values that exceed this value are not
	// valid and MUST NOT be used by servers and clients with schema version 0x020A and
	// earlier. It is defined for simplicity in writing IDL definitions and code. This symbolic
	// constant has a value of 0x0040.
...
type AuthSuite_AuthSuite_Cert struct {
	...
	//
	// It contains fields which are used when selecting a local certificate and validating
	// a remote peer's certificate during certificate authentication.
...
var (
	...
	//
	// FW_CRYPTO_KEY_EXCHANGE_MAX_V2_10 = FW_CRYPTO_KEY_EXCHANGE_DH24:  This value and
	// values that exceed this value are not valid and MUST NOT be used by servers and clients
	// with schema version 0x020A and earlier. It is defined for simplicity in writing IDL
...
var (
	...
	// and MUST NOT be used. It is defined for simplicity in writing IDL definitions and
	// code. This symbolic constant has a value of 1.
...
var (
	...
	// value of 8.
	//
	// FW_PHASE2_CRYPTO_PFS_MAX_V2_10 = FW_PHASE2_CRYPTO_PFS_DH24:  This value and values
	// that exceed this value are not valid and MUST NOT be used by servers and clients
	// with schema version 0x020A and earlier. It is defined for simplicity in writing IDL
	// definitions and code. This symbolic constant has a value of 8.
...
type Phase2SADetails struct {
	...
	PFS Phase2CryptoPFS `idl:"name:Pfs" json:"pfs"`
	// TransportFilterId:  This GUID MAY contain additional implementation-specific<22>
...
type MMRule struct {
	...
	// is enforced by the local computer based on the local computer's platform information.
	// The rule is enforced only if the local computer's operating system platform is an
	// element of the set described by PlatformValidityList.<23>
	PlatformValidityList *OSPlatformList `idl:"name:PlatformValidityList" json:"platform_validity_list"`
	// Origin:  This field is the rule origin, as specified in the FW_RULE_ORIGIN_TYPE enumeration.
	// It MUST be filled on enumerated rules and ignored on input.
	Origin RuleOriginType `idl:"name:Origin" json:"origin"`
